...
$seller_query = "SELECT seller_id FROM sellers WHERE user_id = $check['user_id']";
...
$buyer_query = "SELECT buyer_id FROM buyers WHERE user_id = $check['user_id']";
$buyer_result = mysqli_query($connection,$buyer_query);
$buyer = mysqli_fetch_array($seller_result);
if($buyer_result) {$_SESSION['account_type'] = 'buyer';
$_SESSION['buyer_id']=$buyer['buyer_id'];}  // Buyer ID in session

if(!isset($_SESSION[account_type])) {$_SESSION['account_type']= 'visitor'}   // To Andrew: Defined session's account types and IDs for your usage.
